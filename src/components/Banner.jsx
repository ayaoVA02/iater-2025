import { dec, subscribe_bg } from "../assets/images"

const Banner = () => {
  return (
    <div className="px-2 mb-2">

    <div className=" text-white py-6 px-4 relative overflow-hidden h-48 rounded-3xl " style={{ backgroundImage: `url(${subscribe_bg})` , backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
      <div className="container mx-auto relative z-10 text-center">
        <h2 className="text-xl md:text-2xl font-bold mb-8">Institute of Advanced Technology Education & Research</h2>
        <p className="text-sm opacity-90">
          The era will come when anyone can use AI, even if they do not know the technology. (Bartali Gordon-Pharos AI
          co-founder and CEO)
        </p>
      </div>

      {/* Rocket illustration */}
      <div className="absolute right-4 top-1/2 transform -translate-y-1/2">
      <img src={dec} alt="" />
        <div className="relative w-12 h-12">
          <div className="absolute w-8 h-8 bg-white opacity-20 rounded-full"></div>
          <div className="absolute w-6 h-6 bg-white opacity-30 rounded-full -right-2 -top-2"></div>
       
        </div>
      </div>
    </div>
    </div>
  )
}

export default Banner
